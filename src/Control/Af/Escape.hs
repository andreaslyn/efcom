...
takeEscape ex = Af $ \ sz ar s ->
  let di = shortcutDepthCellIndex @(Escape ex ref) @efs sz
      s' = writeStrictAfArray @Int ar 0# (GHC.I# (shortcutDepth di)) s
...
scopeEscape af f g r = Af $ \ sz ar0 s0 ->
  let di = shortcutDepthCellIndex @(Escape ex ref) @efs sz in
  case copyFromAfArray ar0 (cellIndex di) sz s0 of
    (# s1, backup #) ->
      case unAf af sz ar0 s1 of
        (# ar1, s2, (# e | #) #) ->
          case readAfArray @Int ar1 0# s2 of
            (# s3, d #) ->
              case unI# d GHC.==# shortcutDepth di of
                1# ->
